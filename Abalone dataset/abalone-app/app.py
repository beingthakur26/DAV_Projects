...
app = Flask(__name__)
CORS(app)  # allows your React frontend to call this API from a different domain
...
def predict():
    try:
        ...
        # Sex encoding: Male=0, Female=1, Infant=2
        features = np.array([[
            float(data["sex"]),
            float(data["length"]),
            float(data["diameter"]),
        ...
        # Predict rings (no scaler — model was trained on raw features)
